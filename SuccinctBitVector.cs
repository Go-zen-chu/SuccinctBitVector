...
namespace Succinct
{
    public class SuccinctBitVector<T>
    {
        ...
        static ulong[] BIT_FLG_ARRAY = new ulong[BIT_FLG_ARRAY_LENGTH];
        ...
        static ushort[] BIT_CNT_ARRAY = new ushort[BIT_FLG_ARRAY_LENGTH];
        ...
        public void SetValue(int idx, T value)
        {
            ...
            BIT_FLG_ARRAY[idx / 64] |= bitFlg;

            denseDataList.Add(value);
        }

        // Actually this is an ugly solution because you can just define GetValue method
        // and switch conditions depends on Types like "if(typeof(T) == typeof(int))"
        // but I didn't want to use "if" inside these methods so I changed the name of the methods.
        ...
        public T GetValue_Numeric(int idx)
        {
            ...
            return isDatumExists(idx) ? denseDataList[rank(idx) - 1] : (T)(object)0;
        }
        // This method throws exception if type T is not nullable(e.g. numeric Types)
        public T GetValue_Nullable(int idx)
        {
            return isDatumExists(idx) ? denseDataList[rank(idx) - 1] : (T)(object)null;
        }
        ...
        bool isDatumExists(int idx)
        {
            ...
            var flg = BIT_FLG_ARRAY[idx / 64] & bitFlg;
            return flg != 0x0UL;
        }
        ...
        ushort rank(int idx)
        {
            ...
            return (ushort)(BIT_CNT_ARRAY[idx / 64] + popCount64(BIT_FLG_ARRAY[idx / 64] & bit_mask));
        }

        public void Build()
        {
            // count flags that have true values
            BIT_CNT_ARRAY[0] = 0; // No flg is true before idx 0
            for (int array_idx = 0; array_idx < BIT_FLG_ARRAY_LENGTH - 1; array_idx++)
            {
                BIT_CNT_ARRAY[array_idx + 1] = (ushort)(BIT_CNT_ARRAY[array_idx] + popCount64(BIT_FLG_ARRAY[array_idx]));
            }
        }
